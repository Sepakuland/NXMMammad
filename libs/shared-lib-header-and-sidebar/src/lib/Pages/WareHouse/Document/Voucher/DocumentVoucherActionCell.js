...
const ActionCell = (props) => {
    ...
    const PrintDocumentVoucherRowWC = () => {
        window.open(`/Warehouse/Document/Voucher/PrintDocumentVoucherRowWC?id=${props.dataItem.DocumentCode}`, '_blank', 'noopener,noreferrer');
    }
    const PrintDocumentVoucherRowWOC = () => {
        window.open(`/Warehouse/Document/Voucher/PrintDocumentVoucherRowWOC?id=${props.dataItem.DocumentCode}`, '_blank', 'noopener,noreferrer');
    }
    const Edit = () => {
        navigate(`/Warehouse/Document/Voucher/New?id=${props.dataItem.DocumentId}`, { replace: false });
    }
        ...
        <>
            <td colSpan="1" >
                <div className={`d-flex justify-content-between`} >
                    ...
                    >
                        <MenuItem onClick={PrintDocumentVoucherRowWC}>{t("همراه با قیمت")}</MenuItem>
                        <MenuItem onClick={PrintDocumentVoucherRowWOC}>{t("بدون قیمت")}</MenuItem>
                    </Menu>
                </div>
            </td>
            <Modal
