...
import DocumentDraftActionCell from "./DocumentDraftActionCell";
import { DocumentDraftData } from "./DocumentDraftData";

const DocumentDraftGrid = () => {
    ...
    useEffect(() => {
        ...
        let tempExcel = DocumentDraftData.map((data, index) => {
            let temp = (data.Value.Price).toString().replaceAll(',', '')
            let price = temp !== '' ? parseFloat(temp, 2) : 0
    ...
    const NewDraft = () => {
        navigate(`/Warehouse/Document/Draft/New`, { replace: false });
    }
...
export default DocumentDraftGrid