...
const ActionCell = (props) => {
  ...
  const PrintSelected = () => {
    window.open(
      `/WareHouse/purchase/returnConfirmedPurchases/PrintSelectedWC?id=${props.dataItem.ReversionCode}`,
      "_blank",
      "noopener,noreferrer"
    );
  };
    ...
    <>
      <td colSpan="1">
        <div className={`d-flex justify-content-between`}>
          ...
          <Tooltip title={t("حواله")}>
            <IconButton
              variant="contained"
              className="kendo-action-btn purple-color"
              onClick={() => console.log("view", props.dataItem.ReversionCode)}
            >
              <Link
                to={`${pathname}/Sharing?lang=${i18n.language}&id=1816`}
                // target={"_blank"}
                className={"link-tag"}
              >
                <FileUploadIcon/>
          ...
          <Tooltip title={t("ویرایش")}>
            <IconButton
              ...
              color="info"
              className="kendo-action-btn"
              onClick={() => console.log("edit", props.dataItem.ReversionCode)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
