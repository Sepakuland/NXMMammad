...
import SecoundDistributor from './SecoundDistributor.json'
...
export const Send = () => {
    ...
    const callComponent = () => {
        history.navigate(`/WareHouse/sale/SendCollection`);
    };
    ...
    function tempKeyDownHandler(e) {
        ...
        if (e.keyCode === 40 && tempCashOpen === false) { /* Down Arrowkey */
            e.preventDefault()
        ...
        if (e.keyCode === 38 && tempCashOpen === false) { /* Up ArrowKey */
            e.preventDefault()
        ...
        if (e.keyCode === 39) { /* Right ArrowKey */
            i18n.dir() === "rtl" ? MovePrevious(prev) : MoveNext(formik.values.tempReceived, addTempReceivedRow, next, tempFocusedRow)
        }
        if (e.keyCode === 37) { /* Left ArrowKey */
            i18n.dir() === "ltr" ? MovePrevious(prev) : MoveNext(formik.values.tempReceived, addTempReceivedRow, next, tempFocusedRow)
        }
        if (e.keyCode === 13 && tempCashOpen === false) { /* Enter */
            MoveNext(formik.values.tempReceived, addTempReceivedRow, next, tempFocusedRow)
        }
    ...
    useEffect(() => {
        if (!timeValueSet) {
            var timeUpdate = setInterval(
                () => formik.setFieldValue("time", new Date()),
                30000
            );
        }
        return () => {
                        ...
                        <div className="form-design">
                            <div className="row ">
                                    ...
                                    <div className="wrapper">
                                        <div>
                                            <SelectBox
                                                ...
                                                rtlEnabled={i18n.dir() === "ltr" ? false : true}
                                                onValueChanged={(e) =>
                                                    formik.setFieldValue("Car", e.value)
                                                }
                                                className="selectBox"
                                                noDataText={t("اطلاعات یافت نشد")}
                                                displayExpr={function (item) {
                                    ...
                                    <div className="wrapper">
                                        <div>
                                            <SelectBox
                                                ...
                                                itemRender={null}
                                                placeholder=""
                                                name="SeccoundDistributor"
                                                id="SeccoundDistributor"
                                            />
                                        </div>
                                        {formik.touched.SeccoundDistributor &&
                                            formik.errors.SeccoundDistributor &&
                                            !formik.values.SeccoundDistributor ? (
                                            <div className="error-msg">
                                ...
                                <div content col-lg-12 col-md-12 col-xs-12>
                                    <span className="mt-5">{t("اقلام پیش فاکتور")}:</span>
                                    <RKGrid
                                        gridId={"Send_Page"}
                                        gridData={data}
                                        excelData={excelData}
                                        ...
                                        sortable={false}
                                        pageable={true}
                                        excelFileName={t("اقلام پیش فاکتور")}
                                        reorderable={false}
                                        selectable={false}
                                    />
                                </div>
                                                ...
                                                <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                                                    <div className="content col-lg-12 col-5">
                                                        ...
                                                        >
                                                            ...
                                                            <div className="content col-lg-6 col-6">
                                                                {/* Copyright addTempReceivedRow Grid V3.0
                                                            All rights reserved */}
                                                                                ...
                                                                                render={({ push, remove }) => (
                                                                                    <React.Fragment>
                                                                                        {formik?.values?.tempReceived?.map(
                                                                                            (tempReceives, index) => (
                                                                                                <tr
                                                                                                    key={tempReceives.formikId}
                                                                                                    onFocus={(e) => setTempFocusedRow(e.target.closest("tr").rowIndex)}
                                                                                                    className={tempFocusedRow === index + 1 ? "focus-row-bg" : ""}>
                                                                                                    ...
                                                                                                    <td style={{ minWidth: "120px", }} >
                                                                                                        <input
                                                                                                            ...
                                                                                                            onKeyDown={(e) => tempKeyDownHandler(e)}
                                                                                                            name={`tempReceived.${index}.product`}
                                                                                                            type="text"
                                                                                                            onChange={formik.handleChange}
                                                                                                            value={formik.values.tempReceived[index].product}
                                                                                                            autoComplete="off"
                                                                                                        />
                                                                                                    </td>
                                                                                                    <td style={{ minWidth: "120px", }} >
                                                                                                        <input
                                                                                                            className="form-input w-100"
                                                                                                            onKeyDown={(e) => tempKeyDownHandler(e)}
                                                                                                            name={`tempReceived.${index}.pack`}
                                                                                                            type="text"
                                                                                                            onChange={formik.handleChange}
                                                                                                            value={formik.values.tempReceived[index].pack}
                                                                                                            autoComplete="off"
                                                                                                        />
                                                                                                    </td>
                                                                                                    <td style={{ width: "120px", minWidth: "90px", }}>
                                                                                                        <CurrencyInput
                                                                                                            ...
                                                                                                            decimalsLimit={2}
                                                                                                            onChange={(e) =>
                                                                                                                HandleTempCountChange(index, e.target.value)}
                                                                                                            autoComplete="off" />
                                                                                                    </td>
                                                                                                    <td style={{ width: "120px", minWidth: "90px", }}>
                                                                                                        <CurrencyInput
                                                                                                            ...
                                                                                                            decimalsLimit={2}
                                                                                                            onChange={(e) => HandleTempAmountChange(index, e.target.value)}
                                                                                                            autoComplete="off"
                                                                                                            disabled
                                                                                                        />
                                                                                                    </td>
