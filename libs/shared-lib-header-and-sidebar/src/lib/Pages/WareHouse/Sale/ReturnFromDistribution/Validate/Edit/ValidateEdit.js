...
import { preInvoiceDatagridMeasurementUnitLookup, PreInvoiceGridData, revisionReasons, series } from "./datasources";
...
import { cashDatagridCashLookup, goodGiftDatagridLookup } from "./fereidooniDatasources";
...
const Edit = () => {
    const emptyPreInvoice = { formikId: new Guid().valueOf(), product: '', batchNumber: '', expireDate: '', unit: '', count: 0, quantity: 0, fee: -1, feeChange: 0, amount: 0, discountPercent: 0, discountAmount: 0, brokenDiscount: 0, afterDiscount: 0, VATPercent: 0, VATAmount: 0, sum: 0, description: '', currentStock: 0, previousPurchase: 0 }
    ...
    const cancel = () => {
        history.navigate("/Warehouse/Sale/ReturnFromDist/Validate")
    }
    ...
    const HandlePriceEditChange = (event) => {
        ...
        if (event.target.checked === false) {
            formik.values.preInvoiceItems.forEach((preInvoiceItem, index) => {
                if (typeof (preInvoiceItem.product) === "object") {
                    formik.setFieldValue(`preInvoiceItems[${index}].fee`, preInvoiceItem.product.fee)
                    formik.setFieldValue(`preInvoiceItems[${index}].amount`,
                        (preInvoiceItem.product.fee + parseInt(preInvoiceItem.feeChange)) *
                        parseInt(preInvoiceItem.quantity))

                    formik.setFieldValue(`preInvoiceItems[${index}].afterDiscount`,
                        parseInt(preInvoiceItem.quantity) *
                        (preInvoiceItem.product.fee + parseInt(preInvoiceItem.feeChange)) -
                        parseFloat(preInvoiceItem.discountAmount) -
                        parseFloat(preInvoiceItem.brokenDiscount))

                    formik.setFieldValue(`preInvoiceItems[${index}.VATAmount]`,
                        parseInt(preInvoiceItem.quantity) *
                        (preInvoiceItem.product.fee + parseInt(preInvoiceItem.feeChange))
                        * parseInt(preInvoiceItem.VATPercent) / 100)
                    ...
                    formik.setFieldValue(`preInvoiceItems[${index}].fee`, emptyPreInvoice.fee)
                    formik.setFieldValue(`preInvoiceItems[${index}].amount`,
                        (emptyPreInvoice.fee + parseInt(preInvoiceItem.feeChange)) *
                        parseInt(preInvoiceItem.quantity))
    ...
    const HandleTaxEditChange = (event) => {
        ...
        if (event.target.checked === false) {
            formik.values.preInvoiceItems.forEach((preInvoiceItem, index) => {
                if (typeof (preInvoiceItem.product) === "object") {
                    formik.setFieldValue(`preInvoiceItems[${index}].VATPercent`, preInvoiceItem.product.VATPercent)

                    formik.setFieldValue(`preInvoiceItems[${index}].VATAmount`,
                        (parseInt(preInvoiceItem.afterDiscount) *
                            parseInt(preInvoiceItem.product.VATPercent)) / 100);
                    ...
                    formik.setFieldValue(`preInvoiceItems[${index}].VATPercent`, emptyPreInvoice.VATPercent)

                    formik.setFieldValue(`preInvoiceItems[${index}].VATAmount`,
                        (parseInt(preInvoiceItem.afterDiscount) *
                            parseInt(emptyPreInvoice.VATPercent)) / 100);
    ...
    const HandleDiscountEditChange = (event) => {
        ...
        if (event.target.checked === false) {
            formik.values.preInvoiceItems.forEach((preInvoiceItem, index) => {
                if (typeof (preInvoiceItem.product) === "object") {
                    formik.setFieldValue(`preInvoiceItems[${index}].feeChange`, preInvoiceItem.product.feeChange)
                    formik.setFieldValue(`preInvoiceItems[${index}].amount`,
                        parseInt(preInvoiceItem.quantity)
                        * (parseInt(preInvoiceItem.fee) + preInvoiceItem.product.feeChange));
                    formik.setFieldValue(`preInvoiceItems[${index}].discountPercent`, preInvoiceItem.product.discountPercent)
                    formik.setFieldValue(`preInvoiceItems[${index}].discountAmount`,
                        (parseFloat(preInvoiceItem.amount) * preInvoiceItem.product.discountPercent) / 100 || 0);
                    formik.setFieldValue(`preInvoiceItems[${index}].brokenDiscount`, preInvoiceItem.product.brokenDiscount)

                    formik.setFieldValue(`preInvoiceItems[${index}].afterDiscount`,
                        parseInt(preInvoiceItem.quantity) *
                        (parseInt(preInvoiceItem.fee) + preInvoiceItem.product.feeChange) -
                        ((parseFloat(preInvoiceItem.amount) * preInvoiceItem.product.discountPercent) / 100 || 0) -
                        parseFloat(preInvoiceItem.product.brokenDiscount));
                    ...
                    formik.setFieldValue(`brokenDiscountPercent`, 0)
                    formik.setFieldValue(`brokenDiscountAmount`, 0)
                    formik.setFieldValue(`cashReward`, 0)
                    setPreInvoiceBrokenDiscountTotal(0)
                }
                else {
                    formik.setFieldValue(`preInvoiceItems[${index}].feeChange`, emptyPreInvoice.feeChange)
                    formik.setFieldValue(`preInvoiceItems[${index}].amount`,
                        parseInt(preInvoiceItem.quantity)
                        * (parseInt(preInvoiceItem.fee) + emptyPreInvoice.feeChange));
                    formik.setFieldValue(`preInvoiceItems[${index}].discountPercent`, emptyPreInvoice.discountPercent)
                    formik.setFieldValue(`preInvoiceItems[${index}].discountAmount`,
                        (parseFloat(preInvoiceItem.amount) * emptyPreInvoice.discountPercent) / 100 || 0);
                    formik.setFieldValue(`preInvoiceItems[${index}].brokenDiscount`, emptyPreInvoice.brokenDiscount)

                    formik.setFieldValue(`preInvoiceItems[${index}].afterDiscount`,
                        parseInt(preInvoiceItem.quantity) *
                        (parseInt(preInvoiceItem.fee) + emptyPreInvoice.feeChange) -
                        ((parseFloat(preInvoiceItem.amount) * emptyPreInvoice.discountPercent) / 100 || 0) -
                        parseFloat(emptyPreInvoice.brokenDiscount));
    ...
    function addPreInvoiceItemsRow() {
        ...
        setGridDataItem([...gridDataItem, { product: '', batchNumber: '', expireDate: '', unit: '', count: 0, quantity: 0, fee: -1, feeChange: 0, amount: 0, discountPercent: 0, discountAmount: 0, brokenDiscount: 0, afterDiscount: 0, VATPercent: 0, VATAmount: 0, sum: 0, description: '', currentStock: 0, previousPurchase: 0 }])
    }
                                                            ...
                                                            <React.Fragment>
                                                                {formik.values?.preInvoiceItems?.map((preInvoiceItem, index) => (
                                                                    ...
                                                                    >
                                                                            ...
                                                                            <div className="table-autocomplete">
                                                                                <Autocomplete
                                                                                    ...
                                                                                    onChange={(event, value) => {
                                                                                        RenderPreInvoiceProductOpenState(index, false)
                                                                                        if (value) {
                                                                                            formik.setFieldValue(`preInvoiceItems[${index}].product`, value)
                                                                        ...
                                                                        <td style={{ Width: '40px' }}>
                                                                            <Checkbox
                                                                                checked={preInvoiceItem.VATPercent !== 0}
                                                                                color="success"
                                                                        ...
                                                                        <td style={{ minWidth: "120px" }}>
                                                                            <input
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    if (e.target.value) {
                                                                                        formik.setFieldValue(`preInvoiceItems[${index}].VATPercent`, parseInt(e.target.value))
