...
import { useSearchDetailedAccountQuery } from "../../../features/slices/detailedAccountSlice"
import { CreateQueryString } from "../../../utils/createQueryString"
import objectNotNull from "../../../utils/notNullObject"
...
import GHGrid, { InputGridAutoComplete, InputGridCurrencyInput, InputGridDatepicker, InputGridInput, KeyboardNavigation } from "react-input-grid"


export const InputGrid = ({ formik, creditsTotal, setCreditsTotal, debitsTotal, setDebitsTotal, showArticles, setShowArticles }) => {
    ...
    const { data: detailed4SearchResult = [], isFetching: detailed4SearchIsFetching, error: detailed4SearchError } = useSearchDetailedAccountQuery(detailed4SearchParam
        , {
            skip: detailed4Skip
        }
    );
    ...
    const { data: detailed5SearchResult = [], isFetching: detailed5SearchIsFetching, error: detailed5SearchError } = useSearchDetailedAccountQuery(detailed5SearchParam
        , {
            skip: detailed5Skip
        }
    );
    ...
    const { data: detailed6SearchResult = [], isFetching: detailed6SearchIsFetching, error: detailed6SearchError } = useSearchDetailedAccountQuery(detailed6SearchParam
        , {
            skip: detailed6Skip
        }
    );
        ...
        return (
            <div className={`table-autocomplete position-relative`}>
                <InputGridAutoComplete
                    ...
                    open={documentArticlesFocusedRow === index + 1 ? moeinAccountOpen : false}
                    ...
                    onChange={(event, value) => {
                        RenderMoeinAccountOpenState(index, false)
                        formik.setFieldValue(`documentArticles[${index}].moeinAccountId`, value)
                        detailed4SearchFormik.setFieldValue(`DetailedTypeID`, value.detailedType4Ids)
                        detailed5SearchFormik.setFieldValue(`DetailedTypeID`, value.detailedType5Ids)
                        detailed6SearchFormik.setFieldValue(`DetailedTypeID`, value.detailedType6Ids)
                        var test = value.formersNames.split("/")
                        setShowArticles({ ...showArticles, group: test[0].trim(), kol: test[1].trim(), moein: value.name })
                    }}
                    onBlur={(e) => {
                        RenderMoeinAccountOpenState(index, false)
                    }}
                    onKeyDown={(e) => {
                        if ((e.keyCode === 13 || e.keyCode === 9 || e.keyCode === 38 || e.keyCode === 40 || e.keyCode === 37 || e.keyCode === 39) && moeinAccountOpen === false) {    /* Enter */
                            e.preventDefault()
                            RenderMoeinAccountOpenState(index, false)
                        }
                        setTimeout(() => {
                            KeyboardNavigation(e, [moeinAccountOpen, detailedOpen4, detailedOpen5, detailedOpen6], formik.values.documentArticles, documentArticlesFocusedRow, addDocumentArticlesRow, i18n.dir())
                        }, 0);
                    }}
                />
                <Tooltip
                    title={t("ایجاد حساب معین")}>
                    <IconButton
                        aria-label={'Clear'}
                        variant="contained"
                        color={"success"}
        ...
        return (
            <div className={`table-autocomplete position-relative`}>
                <InputGridAutoComplete
                    ...
                    innerWidth={300}
                    backgroundColor={formik.values.documentArticles[index].moeinAccountId?.detailedType4Ids?.length == 0 || !objectNotNull(formik.values.documentArticles[index].moeinAccountId) ? '#EFEFEF4D' : '#FFFFFF'}
                    value={formik.values.documentArticles[index].detailed4Id}
                    disabled={formik.values.documentArticles[index].moeinAccountId?.detailedType4Ids?.length == 0 || !objectNotNull(formik.values.documentArticles[index].moeinAccountId)}
                    renderOption={(props, option) => (
                        <Box
                    ...
                    open={documentArticlesFocusedRow === index + 1 ? detailedOpen4 : false}
                    ...
                    onChange={(event, value) => {
                        RenderDetailedOpenState4(index, false)
                        formik.setFieldValue(`documentArticles[${index}].detailed4Id`, value)
                        setShowArticles({ ...showArticles, detailed4: value.detailedAccountName })
                    }}
                    onBlur={(e) => RenderDetailedOpenState4(index, false)}
                    onKeyDown={(e) => {
                        if ((e.keyCode === 13 || e.keyCode === 9 || e.keyCode === 38 || e.keyCode === 40 || e.keyCode === 37 || e.keyCode === 39) && detailedOpen4 === false) {
                            e.preventDefault()
                            RenderDetailedOpenState4(index, false)
                        }
                        setTimeout(() => {
                ...
                <Tooltip
                    ...
                    <IconButton
                        ...
                        sx={i18n.dir() === 'rtl' ? { left: 0 } : { right: 0 }}>
                        <Link
                            to={'/baseInformation/accounting/entity'}
                            target={"_blank"}
                        >
                            <AddCircleOutlineIcon fontSize="small" />
                        </Link>
        ...
        return (
            <div className={`table-autocomplete position-relative`}>
                <InputGridAutoComplete
                    ...
                    innerWidth={300}
                    backgroundColor={formik.values.documentArticles[index].moeinAccountId?.detailedType5Ids?.length == 0 || !objectNotNull(formik.values.documentArticles[index].moeinAccountId) ? '#EFEFEF4D' : '#FFFFFF'}
                    value={formik.values.documentArticles[index].detailed5Id}
                    disabled={formik.values.documentArticles[index].moeinAccountId?.detailedType5Ids?.length == 0 || !objectNotNull(formik.values.documentArticles[index].moeinAccountId)}
                    renderOption={(props, option) => (
                        <Box
                            component="li" {...props} key={option.detailedAccountId}>
                            {option.detailedAccountCode}-({option.detailedAccountName})
                    ...
                    open={documentArticlesFocusedRow === index + 1 ? detailedOpen5 : false}
                    ...
                    onChange={(event, value) => {
                        RenderDetailedOpenState5(index, false)
                        formik.setFieldValue(`documentArticles[${index}].detailed5Id`, value)
                        setShowArticles({ ...showArticles, detailed5: value.detailedAccountName })
                    }}
                    onBlur={(e) => RenderDetailedOpenState5(index, false)}
                    onKeyDown={(e) => {
                        if ((e.keyCode === 13 || e.keyCode === 9 || e.keyCode === 38 || e.keyCode === 40 || e.keyCode === 37 || e.keyCode === 39) && detailedOpen5 === false) {
                            e.preventDefault()
                            RenderDetailedOpenState5(index, false)
                        }
                        setTimeout(() => {
        ...
        return (
            <div
                ...
                <InputGridAutoComplete
                    ...
                    innerWidth={300}
                    backgroundColor={formik.values.documentArticles[index].moeinAccountId?.detailedType6Ids?.length == 0 || !objectNotNull(formik.values.documentArticles[index].moeinAccountId) ? '#EFEFEF4D' : '#FFFFFF'}

                    value={formik.values.documentArticles[index].detailed6Id}
                    disabled={formik.values.documentArticles[index].moeinAccountId?.detailedType6Ids?.length == 0 || !objectNotNull(formik.values.documentArticles[index].moeinAccountId)}
                    renderOption={(props, option) => (
                        <Box
                            component="li" {...props} key={option.detailedAccountId}>
                            {option.detailedAccountCode}-({option.detailedAccountName})
                    ...
                    open={documentArticlesFocusedRow === index + 1 ? detailedOpen6 : false}
                    ...
                    onChange={(event, value) => {
                        RenderDetailedOpenState6(index, false)
                        formik.setFieldValue(`documentArticles[${index}].detailed6Id`, value)
                        setShowArticles({ ...showArticles, detailed6: value.detailedAccountName })
                    }}
                    onBlur={(e) => RenderDetailedOpenState6(index, false)}
                    onKeyDown={(e) => {
                        if ((e.keyCode === 13 || e.keyCode === 9 || e.keyCode === 38 || e.keyCode === 40 || e.keyCode === 37 || e.keyCode === 39) && detailedOpen6 === false) {
                            e.preventDefault()
                            RenderDetailedOpenState6(index, false)
                        }
                        setTimeout(() => {
    ...
    const debitsCell = (index) => {
        return (
            <InputGridCurrencyInput
                onKeyDown={(e) => KeyboardNavigation(e, [moeinAccountOpen, detailedOpen4, detailedOpen5, detailedOpen6], formik.values.documentArticles, documentArticlesFocusedRow, addDocumentArticlesRow, i18n.dir())}
                id="debits"
                name={`documentArticles.${index}.debits`}
                value={formik.values.documentArticles[index].debits}
                onValueChange={(value) => formik.setFieldValue(`documentArticles[${index}].debits`, value)}
        ...
        return (
            <IconButton
                ...
                onClick={() => {
                    ...
                    formik.setFieldValue(`documentArticles[${index}].credits`, formik.values.documentArticles[index].debits)
    ...
    const creditsCell = (index) => {
        return (
            <InputGridCurrencyInput
                ...
                onValueChange={(value) => formik.setFieldValue(`documentArticles[${index}].credits`, value)}
    ...
    const trackingNumberCell = (index) => {
        return (
            <InputGridInput
                disabled={formik.values.documentArticles[index].moeinAccountId?.followUpCharacteristics === false || !objectNotNull(formik.values.documentArticles[index].moeinAccountId)}
                onKeyDown={(e) => KeyboardNavigation(e, [moeinAccountOpen, detailedOpen4, detailedOpen5, detailedOpen6], formik.values.documentArticles, documentArticlesFocusedRow, addDocumentArticlesRow, i18n.dir())}
        ...
        return (
            ...
            >
                <InputGridDatepicker
                    ...
                    onChange={(date) => {
                        if (date !== null) {
                            formik.setFieldValue(`documentArticles[${index}].trackingDate`, julianIntToDate(date.toJulianDay()));
                        }
                    }}
                />
            </div>
        ...
        return (
            <div
                ...
                <div
                    ...
                    <div
                        className={renderBalanceClassName()}>
                        <label style={{
                            display: "flex",
                            alignItems: "center",
                            textAlign: "center",
                    ...
                    {formik.touched.balance && formik.errors.balance ? (
                        <div className='error-msg'>{t(formik.errors.balance)}</div>) : null}
                </div>
            </div>
        )
    }
    ...
    return (
        <GHGrid
            ...
            removeRowOperation={(index) => {
                setCreditsTotal(creditsTotal - parseFloat(formik.values.documentArticles[index].credits))
                setDebitsTotal(debitsTotal - parseFloat(formik.values.documentArticles[index].debits))
            }}
            rowFocusFunction={(e) => {
                var codingNames = ['', '', '']
                var rowIndex = e.target.closest("tr").rowIndex
                setDocumentArticlesFocusedRow(rowIndex)
                if (objectNotNull(formik.values.documentArticles[rowIndex - 1].moeinAccountId)) {
                    codingNames = formik.values.documentArticles[rowIndex - 1].moeinAccountId?.formersNames.split("/")
                }
                setShowArticles({
                    ...showArticles,
                    ...
                    detailed4: formik.values.documentArticles[rowIndex - 1].detailed4Id?.detailedAccountName || '',
                    detailed5: formik.values.documentArticles[rowIndex - 1].detailed5Id?.detailedAccountName || '',
                    detailed6: formik.values.documentArticles[rowIndex - 1].detailed6Id?.detailedAccountName || ''
                })
            }}
