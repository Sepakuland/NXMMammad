...
import shoppingData from "./OpenWareHouseConfirmation/dataForShopping.json";
...
export default function ReceiptDocument() {
  ...
  function purchaseKeyDownHandler(e) {
    let next = e.target.closest("td").nextSibling;
    while (
      next.cellIndex !== next.closest("tr").children.length - 1 &&
      (next.querySelector("button") || next.querySelector("input").disabled)
    ...
    if (
      ...
      /* Down Arrowkey */
      e.preventDefault();
      if (formik.values.purchaseReceived.length === purchaseFocusedRow) {
  ...
  const callComponent = () => {
    history.navigate(`/Accounting/wareHouseConfirmation/wareHouseEntrance`);
  };
