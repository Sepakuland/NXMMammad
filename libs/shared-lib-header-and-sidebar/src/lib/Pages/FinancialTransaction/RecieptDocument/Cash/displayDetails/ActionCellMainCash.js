...
const ActionCellMainCash = (props) => {
    ...
    const PrintSelected = () => {
        window.open(`/FinancialTransaction/receiptDocument/Cash/DisplayDetails/PrintSelected?id=${props.dataItem.DocumentCode}`, '_blank' );
    }
    const EditMultiPlexing = () => {
        history.navigate(`/FinancialTransaction/receiptDocument/Cash/EditMultiPlexing?id=1805`)
    }
    return (
        <>
            <td colSpan="1" >
                <div className={`d-flex justify-content-between`} >
                    <Tooltip title={t("تسهیم")}>
                        <IconButton variant="contained" className='kendo-action-btn purple-color' onClick={EditMultiPlexing}>
                            <PieChartTwoToneIcon />
                        </IconButton >
                    </Tooltip>
                    <Tooltip title={t("چاپ")}>
...
export default React.memo(ActionCellMainCash)
