...
const ActionCellMainCheque = (props) => {
  ...
  const PrintSelected = () => {
    window.open(
      `/FinancialTransaction/PaymentDocument/Cheque/DisplayDetails/PrintSelected?id=${props.dataItem.DocumentCode}`,
      "_blank"
    );
  };
        ...
        <div className={`d-flex justify-content-between`}>
          ...
          <Tooltip title={t("تسهیم")}>
            <IconButton
              ...
              onClick={() => console.log("view", props.dataItem.DocumentCode)}
            >
              <Link
                to={`${pathname}/Sharing?lang=${i18n.language}&id=1815`}
                // target={"_blank"}
                className={"link-tag"}
              >
                <PieChartTwoToneIcon />
