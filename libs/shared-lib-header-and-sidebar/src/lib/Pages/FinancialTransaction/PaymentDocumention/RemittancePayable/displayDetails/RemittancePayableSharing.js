...
import { definedAccountLookupData, descriptiveLookupData } from "./lookupData";
...
const emptyArticlesTouch = {
  definedAccount: false,
  descriptive: false,
  price: false,
};
...
export default function KaraDataGrid() {
      ...
      balance: Yup.number().when("initialBalance", (initialBalance) => {
        if (parseFloat(initialBalance) !== parseFloat(formik.values.balance))
          return Yup.number()
            .min(
              initialBalance,
              "جمع مبلغ موارد وارد شده با کل مبلغ پرداختی یکسان نیست."
            )
            .max(
              initialBalance,
              "جمع مبلغ موارد وارد شده با کل مبلغ پرداختی یکسان نیست."
  ...
  function keyDownHandler(e, index, currentElm, nextElm, previousElm) {
    if (
      e.keyCode === 40 &&
      definedAccountOpen[index] === false &&
      descriptiveOpen[index] === false
    ) {
      /* Down Arrowkey */
      e.preventDefault();
    ...
    if (
      ...
      /* Enter */ /* MUST BECOME LANGUAGE DEPENDANT */
      e.preventDefault();
      MoveForward(
        formik.values.documentArticles,
        addDocumentArticlesRow,
  ...
  function selectItem(e) {
    if (!e.itemData.hasOwnProperty("items")) {
      ...
      formik.setFieldValue(
        `documentArticles[${documentArticlesFocusedRow - 1}].definedAccount`,
        e.itemData.Code
      );
    }
  }
  ...
  return (
    <>
                          ...
                          <div className="row align-items-center">
                            <div className="content col-md-4 col-12">
                              <div className="title mb-0">
                                <span className="span">
                                  {" "}
                                  {t("طرف حساب های این پرداخت خزانه داری")}:
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="content col-lg-12 col-12">
                                ...
                                <table className="table table-bordered">
                                  ...
                                  <tbody>
                                    <FieldArray
                                      ...
                                      render={({ push, remove }) => (
                                        <React.Fragment>
                                          {formik?.values?.documentArticles?.map(
                                            (documentArticles, index) => (
                                              <tr
                                                key={index}
                                                onFocus={(e) =>
                                                  setDocumentArticlesFocusedRow(
                                                    e.target.closest("tr")
                                                      .rowIndex
                                                  )
                                                }
                                                  ...
                                                  >
                                                    <Autocomplete
                                                      ...
                                                      onChange={(
                                                        ...
                                                        formik.setFieldValue(
                                                          `documentArticles[${index}].definedAccount`,
                                                          value.Code
                                                        );
                                                      }}
                                                      onBlur={(e) => {
                                                  ...
                                                  >
                                                    <Autocomplete
                                                      ...
                                                      onChange={(
                                                        ...
                                                        formik.setFieldValue(
                                                          `documentArticles[${index}].descriptive`,
                                                          value.Code
                                                        );
                                                      }}
                                                      onBlur={(e) =>
                                                ...
                                                <td style={{ width: "40px" }}>
                                                  <IconButton
                                                    ...
                                                    onClick={() => {
                                                      ...
                                                      let temp2 =
                                                        definedAccountOpen.filter(
                                                          (item, iIndex) =>
                                                            iIndex !== index
                                                        );
                                                      setDefinedAccountOpen([
                                                        ...temp2,
                                                      ]);
                                                      let temp3 =
                                                        descriptiveOpen.filter(
                                                          (item, iIndex) =>
                                                            iIndex !== index
                                                        );
                                                      setDescriptiveOpen([
                              ...
                              {formik?.errors?.documentArticles?.map(
                                (error, index) => (
                                  <p className="error-msg" key={index}>
                                    {error
                                      ? ` ${t("ردیف")} ${index + 1} : ${error?.definedAccount
                                        ? t(error.definedAccount)
                                        : ""
                                      }${error?.debits && error?.definedAccount
                                        ? "."
                                        : ""
                                      } ${error?.debits ? t(error.debits) : ""
                                      }${error?.credits ? "." : ""} ${error?.credits ? t(error.credits) : ""
                                      }`
                                      : null}
                                  </p>
                                )
      ...
      <div>
        ...
        {/*<Dialog open={errorDialogState} dir={i18n.dir()} >*/}

        {/*    <div className={`modal-header ${i18n.dir() === "ltr" ? 'header-ltr' : ''}`}>*/}
        {/*        <h2>{t("خطا")}</h2>*/}
        {/*        <IconButton variant="contained" color="error" className='kendo-action-btn' onClick={() => setErrorDialogState(false)}><CloseIcon /></IconButton>*/}
        {/*    </div>*/}
        {/*    <DialogContent>*/}
