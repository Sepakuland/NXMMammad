...
const ActionCell = (props) => {
  ...
  const PrintSelectedWC = () => {
    window.open(
      `/Sell/sellReversion/PrintSelectedWC?id=${props.dataItem.InvoiceNumber}`,
      "_blank",
    );
  };
  const PrintSelectedWOC = () => {
    window.open(
      `/Sell/sellReversion/PrintSelectedWOC?id=${props.dataItem.InvoiceNumber}`,
      "_blank",
    );
  };
  ...
  const PrintSelectedOI = () => {
    window.open(
      `/Sell/sellReversion/PrintSelectedOI?id=${props.dataItem.InvoiceNumber}`,
      "_blank"
    );
  };
  const PrintSelectedUOI = () => {
    window.open(
      `/Sell/sellReversion/PrintSelectedUOI?id=${props.dataItem.InvoiceNumber}`,
      "_blank"
    );
  };
    ...
    <>
      <td colSpan="1">
        ...
        <Tooltip title={t("چاپ")}>
          ...
          >
            <MenuItem onClick={PrintSelectedOI}>{t("فاکتور رسمی")}</MenuItem>
            <MenuItem onClick={PrintSelectedUOI}>
              {t("فاکتور غیررسمی")}
            </MenuItem>
          </Menu>
          <Tooltip title={t("چاپ رسید")}>
            <IconButton
              variant="contained"
              style={{ color: "#1890ff" }}
              className="kendo-action-btn"
