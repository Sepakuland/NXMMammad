...
import Visitors from '../../WareHouse/Sale/ReturnFromDistribution/Validate/Edit/Visitors.json'
import AccountPartyData from '../../WareHouse/Sale/ReturnFromDistribution/Validate/Edit/AccountPartyData.json'
...
import { preInvoiceDatagridMeasurementUnitLookup, PreInvoiceGridData, series } from "../../WareHouse/Sale/ReturnFromDistribution/Validate/Edit/datasources";
...
import { cashDatagridCashLookup, goodGiftDatagridLookup } from "../../WareHouse/Sale/ReturnFromDistribution/Validate/Edit/fereidooniDatasources";
import Guid from 'devextreme/core/guid';
import { findNextFocusable, findPreviousFocusable, MoveNext, MovePrevious } from "../../../utils/gridKeyboardNav3";
import { useLocation } from "react-router";
import { Link } from "react-router-dom";
import { receivingWarehouses } from '../../Purchase/ApprovedReturnedPurchase/datasources'
import { storeKeepers } from "../../WareHouse/ApprovedInvoices/Send/datasources";
import settleList from './settleList.json'
...
const InvoiceForm = () => {
    ...
    function CalculatePreInvoiceCountTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceCountTemp += element.count || 0;
    ...
    function CalculatePreInvoiceQuantityTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceQuantityTemp += element.quantity || 0;
    ...
    function CalculatePreInvoiceAmountTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceAmountTemp += element.amount || 0;
    ...
    function CalculatePreInvoiceDiscountAmountTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceDiscountAmountTemp += element.discountAmount || 0;
    ...
    function CalculatePreInvoiceBrokenDiscountTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceBrokenDiscountTemp += element.brokenDiscount || 0;
    ...
    function CalculatePreInvoiceAfterDiscountTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceAfterDiscountTemp += element.afterDiscount || 0;
    ...
    function CalculatePreInvoiceVATAmountTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceVATAmountTemp += element.VATAmount || 0;
    ...
    function CalculatePreInvoiceSumTotal() {
        ...
        formik.values.preInvoiceItems.forEach((element) => {
            preInvoiceSumTemp += element.sum || 0;
    ...
    function HandleGiftNumberChange(index, value) {
        let temp = value.replaceAll(",", "");
        formik.setFieldValue(`giftReceived[${index}].number`, value !== '' ? parsFloatFunction(temp, 2) : 0);
    }

    function HandleGiftAmountChange(index, value) {
        let temp = value.replaceAll(",", "");
        formik.setFieldValue(`giftReceived[${index}].amount`, value !== '' ? parsFloatFunction(temp, 2) : 0);
    ...
    function CalculateGiftNumberTotal() {
        ...
        formik.values.giftReceived.forEach((element) => {
            giftNumberTemp += element.number || 0;
    ...
    function CalculateGiftAmountTotal() {
        ...
        formik.values.giftReceived.forEach((element) => {
            giftAmountTemp += element.amount || 0;
        });
        setGiftAmountTotal(parsFloatFunction(giftAmountTemp, 2));
    }

    console.log('formik.values.giftReceived', formik.values.giftReceived)

    function CalculateGiftPriceTotal() {
        let giftPriceTemp = 0;
        formik.values.giftReceived.forEach((element) => {
            giftPriceTemp += element.price || 0;
            ...
            <div style={{ backgroundColor: `${theme.palette.background.paper}`, padding: '20px' }} >
                <FormikProvider value={formik}>
                    <form onSubmit={formik.handleSubmit}>
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <td style={{ width: "40px" }}>
                                                                            <IconButton
                                                                                ...
                                                                                className="kendo-action-btn"
                                                                                onClick={() => {
                                                                                    setPreInvoiceCountTotal(preInvoiceCountTotal => preInvoiceCountTotal - formik.values.preInvoiceItems[index].count);
                                                                                    setPreInvoiceQuantityTotal(preInvoiceQuantityTotal => preInvoiceQuantityTotal - formik.values.preInvoiceItems[index].quantity);
                                                                                    setPreInvoiceAmountTotal(preInvoiceAmountTotal => preInvoiceAmountTotal - formik.values.preInvoiceItems[index].amount);
                                                                                    setPreInvoiceDiscountAmountTotal(preInvoiceDiscountAmountTotal => preInvoiceDiscountAmountTotal - formik.values.preInvoiceItems[index].discountAmount);
                                                                                    setPreInvoiceBrokenDiscountTotal(preInvoiceBrokenDiscountTotal => preInvoiceBrokenDiscountTotal - formik.values.preInvoiceItems[index].brokenDiscount);
                                                                                    setPreInvoiceAfterDiscountTotal(preInvoiceAfterDiscountTotal => preInvoiceAfterDiscountTotal - formik.values.preInvoiceItems[index].afterDiscount);
                                                                                    setPreInvoiceVATAmountTotal(preInvoiceVATAmountTotal => preInvoiceVATAmountTotal - formik.values.preInvoiceItems[index].VATAmount);
                                                                                    setPreInvoiceSumTotal(preInvoiceSumTotal => preInvoiceSumTotal - formik.values.preInvoiceItems[index].sum);
                                                                                    // let temp=gridDataItem.filter(item=>item.)
                                                                                    let temp1 = gridDataItem.filter((item, iIndex) => iIndex !== index)
                                                                                    setGridDataItem([...temp1])
                                                        ...
                                                        render={({ push, remove }) => (
                                                            <React.Fragment>
                                                                {formik?.values?.giftReceived?.map(
                                                                    (giftReceives, index) => (
                                                                        <tr key={giftReceives.formikId} className={giftFocusedRow === index + 1 ? "focus-row-bg" : ""}
                                                                            ...
                                                                            <td style={{ width: "120px", minWidth: "90px" }}>
                                                                                <CurrencyInput
                                                                                    ...
                                                                                    decimalsLimit={2}
                                                                                    onChange={(e) => {
                                                                                        HandleGiftAmountChange(index, e.target.value)
                                                                                    }}
                        ...
                        <div className='row mt-5 form-design'>
                            ...
                            <div className='col-lg-4 col-md-4 col-12'>
                                ...
                                <div className="wrapper">
                                    <SelectBox
                                        ...
                                        rtlEnabled={i18n.dir() === "ltr" ? false : true}
                                        onValueChanged={(e) => formik.setFieldValue('Warehouse', e.value)}
                                        className='selectBox'
                                        noDataText={t("اطلاعات یافت نشد")}
                                        displayExpr={function (item) {
                                            return (
                                        ...
                                        searchEnabled
                                        showClearButton
                                        disabled={!formik.values.Shipping}
                                    />
                                </div>
                            </div>
