...
import { ParseDocumentStatesEnum, documentStates } from "@kara-erp/shared-lib-header-and-sidebar";
import swal from "sweetalert";
import {getLangDate} from "rkgrid";
...
const DocumentGrid = () => {
  ...
  }, [accountingDocumentIsFetching, accountingDocumentCurrentData]);
  ...
  async function getAllData() {
    ...
    await filterDocument({ obj: obj, filter: filterList })
        .unwrap()
        .then((res) => {
          let tempData = res?.data.map((data,index) => {
            return {
              ...data,
              IndexCell: index + 1,
              documentDate: data.documentDate ? getLangDate(i18n.language, data.documentDate) : '',
              documentState: t(ParseDocumentStatesEnum(data.documentState)),
              originalDocumentState: data.documentState
            };
          });
  ...
  const chartObj = [
    { value: "documentNumber", title: t("ش سند") },
    { value: "folioNumber", title: t("ش عطف") },
    { value: "dailyNumber", title: t("ش روزانه") },
  ];
